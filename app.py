...
"""
    Enregistrer une ligne dans Answer FREE
"""
...
"""
    ...
    ca lit chaque ligne des utilisateurs et 
    ajjoute a une liste 
"""
def users() :
    liste=[]
...
"""
    Recuperer les reponses possibles la liste des questions

"""
def questions() :
    liste=[]
    with open( "data/matching_questions.csv")  as file_user:
        #on lit le fichier  essaie de remplacer lile user par file question
        ...
        for row in f_csv:
            qcm = row[0].split(';')
            #on recupere l'ensemble des reponses avec getqcm qui recupere les reponse
            #avec l'id de la question en parametre (id is qcm[0])
            answers = get_qcm(qcm[0])
            #on ajoute answers a la question
            qcm.append(answers)
            #on ajoute la question a la liste
            liste.append(qcm)
...
def insert_data(data) : 
    #data c'est la liste des reponses du formulaire web

    for k, v in data.items():
        #oon fait une interaction key value sur les donné 
        #Si c'est pas l'utilisateur et le type est qcm
...
def answer():
    ...
    #insert les data recuperer avec request.form
